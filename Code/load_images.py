...
def pickle_images_labels():
	images_labels = []
	# Get all unique gesture class names (folder names)
	gesture_folders = sorted([f for f in os.listdir('gestures') if os.path.isdir(os.path.join('gestures', f))])
	class_mapping = {folder_name: i for i, folder_name in enumerate(gesture_folders)}

	images = glob("gestures/*/*.jpg")
	images.sort()
	for image in images:
		print(image)
		label_str = image[image.find(os.sep)+1: image.rfind(os.sep)]
		# Map the string label to a contiguous integer
		mapped_label = class_mapping[label_str]
		img = cv2.imread(image, 0)
		images_labels.append((np.array(img, dtype=np.uint8), mapped_label))
